...
class LiqHeatMapCurrencyModel2(AsyncSpider):
    name = "liq_heat_map_currency_model_2"
    script_type = "async"
    log = None
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.url = f"{self.domin}/api/index/v3/aggregate/liqHeatMap"
        self.headers = {
    ...
    async def fetch_batch(self):
        ...
        async with AsyncClient(proxy=proxy, headers=self.headers) as client:
            ...
            self._print(f"[{self.name}] fetch batch [{len(tasks)}] tasks")
            for task in asyncio.as_completed(tasks):
                ...
                if (result := item.get("result")) and (data := decrypt_from_app(
    ...
    @auto_execute_method()
    async def liq_heat_map_currency_model_2(self):
...
if __name__ == '__main__':
    ...
    asyncio.run(async_func_test(LiqHeatMapCurrencyModel2, kw))
