...
class LiqHeatMapTraPairModel1(AsyncSpider):
    name = "liq_heat_map_tra_pair_model_1"
    script_type = "async"
    log = None
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.url = f"{self.domin}/api/index/v2/liqHeatMap"
        self.headers = {
    ...
    async def fetch_batch(self):
        ...
        async with AsyncClient(proxy=proxy, headers=self.headers) as client:
            ...
            for task in asyncio.as_completed(tasks):
                ...
                t = self.time_interval_reverse.get(
                    f"{params['interval']}-{params['limit']}"
                )
    ...
    @auto_execute_method()
    async def liq_heat_map_tra_pair_model_1(self):
...
if __name__ == '__main__':
    kw = {
        "extra_params": {
            "exchanges": ["binance"],
            "time_interval": ["12h", "1d", "2d"],
            "tra_pairs": [
            ]
        },
    }
    asyncio.run(async_func_test(LiqHeatMapTraPairModel1, kw))
